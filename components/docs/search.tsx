...
function useAutocomplete({ close }: { close: () => void }) {
  ...
  let [autocomplete] = useState<Autocomplete>(() =>
    ...
    >({
      ...
      getSources({ query }) {
        return import('@/lib/mdx/search.mjs').then((withSearch: any) => {
          return [
            {
              sourceId: 'documentation',
              getItems() {
                return withSearch.search(query, { limit: 5 })
              },
              getItemUrl({ item }) {
                return item.url
              },
...
function SearchResult({
  ...
  let sectionTitle = navigation.find((section) =>
    section.links.find((link: any) => link.href === result.url.split('#')[0])
  )?.title
  ...
  return (
    <li
      className={clsx(
        'group block cursor-default px-4 py-3 bg-background',
  ...
  if (collection.items.length === 0) {
    return (
      <div className='p-6 text-center bg-black'>
        <NoResultsIcon className='mx-auto h-5 w-5 stroke-white' />
        <p className='mt-2 text-xs '>
          No encontramos resultados para:{' '}
          <strong className='break-words font-semibold '>
            &lsquo;{query}&rsquo;
          </strong>
          . Intente con otro query.
        </p>
      </div>
    )
  }
  ...
  return (
    <div className='group relative flex h-12'>
      ...
      <input
        ref={inputRef}
        className={clsx(
          'flex-auto bg-background appearance-none pl-10 text-zinc-900 outline-none placeholder:text-zinc-500 focus:w-full focus:flex-none dark:text-white sm:text-sm [&::-webkit-search-cancel-button]:hidden [&::-webkit-search-decoration]:hidden [&::-webkit-search-results-button]:hidden [&::-webkit-search-results-decoration]:hidden',
    ...
    >
      ...
      >
        ...
        <div className='fixed inset-0 overflow-y-auto px-4 py-4 sm:px-6 sm:py-20 md:py-32 lg:px-8 lg:py-[15vh]'>
          ...
          >
            <Dialog.Panel className='mx-auto transform-gpu overflow-hidden rounded-lg  shadow-xl ring-1 ring-zinc-900/7.5  dark:ring-zinc-800 sm:max-w-xl'>
